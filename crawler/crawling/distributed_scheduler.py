...
class DistributedScheduler(object):
    ...
    global_page_per_domain_filter = (
        None  # the global redis page per domain filter, applied to all domains.
    )
            ...
            if "domains" in loaded_config:
                for domain in loaded_config["domains"]:
                    ...
                    if "window" in item and "hits" in item:
                        ...
                        self.logger.info(
                            "Added domain {dom} to loaded config".format(dom=domain)
                        )
    ...
    def update_domain_queues(self):
        ...
        for key in self.domain_config:
            final_key = "{name}:{domain}:queue".format(
                name=self.spider.name, domain=key
            )
    ...
    def create_queues(self):
        ...
        for key in self.queue_keys:
            ...
            if key not in self.queue_dict or newConf:
                self.logger.info("Added new Throttled Queue {q}".format(q=key))
                ...
                # use custom window and hits
                    ...
                    if "scale" in self.domain_config[the_domain]:
                        hits = int(
                            hits
                            * self.fit_scale(self.domain_config[the_domain]["scale"])
                        )
    ...
    def update_ipaddress(self):
        ...
        try:
            obj = urllib.request.urlopen(
                settings.get("PUBLIC_IP_URL", "http://ipv4.icanhazip.com/")
            )
        ...
        if self.old_ip != self.my_ip:
            self.logger.info(
                "Changed Public IP: {old} -> {new}".format(
    ...
    def report_self(self):
        ...
        key = "stats:crawler:{m}:{s}:{u}".format(
            m=socket.gethostname(), s=self.spider.name, u=self.my_uuid
        )
    ...
    def open(self, spider):
        ...
        self.dupefilter = RFPDupeFilter(
            self.redis_conn, self.spider.name + ":dupefilter", self.rfp_timeout
        )
    ...
    def enqueue_request(self, request):
        ...
        if req_dict["meta"][
            "domain_max_pages"
        ] and self.domain_max_page_filter.request_page_limit_reached(
        ...
        if (
            self.global_page_per_domain_limit
            and self.global_page_per_domain_filter.request_page_limit_reached(
                request=request, spider=self.spider
            )
        ):
            self.logger.info(
        ...
        if not self.is_blacklisted(
            req_dict["meta"]["appid"], req_dict["meta"]["crawlid"]
        ):
            ...
            if (
                self.backlog_blacklist
                or (not self.backlog_blacklist and domain not in self.black_domains)
            ) and (
                req_dict["meta"]["expires"] == 0
                or curr_time < req_dict["meta"]["expires"]
            ):
                # we may already have the queue in memory
                if key in self.queue_keys:
                    self.queue_dict[key][0].push(req_dict, req_dict["meta"]["priority"])
                else:
                    ...
                    self.redis_conn.zadd(
                        key, {ujson.dumps(req_dict): -req_dict["meta"]["priority"]}
                    )
                self.logger.info(
    ...
    def next_request(self):
        ...
        if item:
            self.logger.info(self.hits)
            self.logger.info(self.window)
            self.logger.info("Found url to crawl {url}".format(url=item["url"]))
            if "meta" in item:
