...
class BayoutCondoDetails(RedisSpider):
    ...
    def parse(self, response):
        ...
        apInfo.surface = getNumberFromStr(selectSoupElement('surface', soup, SoupSearchObj('span', {"aria-label": "Area"}, True, 0), ['span', 'span', 'text']))

        apInfo.roomsNo = getNumberFromStr(selectSoupElement('roomsNo', soup, SoupSearchObj('span', {"aria-label": "Beds"}, True, 0),
                                           ['span', 'text']))

        apInfo.bathroomsNo = getNumberFromStr(
            selectSoupElement('bathroomsNo', soup, SoupSearchObj('span', {"aria-label": "Baths"}, True, 0),
                              ['span', 'text']))

        apInfo.type = selectSoupElement('type', soup, SoupSearchObj('span', {"aria-label": "Type"}, True, 0),
                              [ 'text'])

        apInfo.createdOn = selectSoupElement('createdOn', soup, SoupSearchObj('span', {"aria-label": "Reactivated date"}, True, 0),
                                        ['text'])
        apInfo.zone = selectSoupElement('zone', soup, SoupSearchObj('div', {"aria-label": "Property header"}, True, 0),
                                        ['text'])

        apInfo.price = getNumberFromStr(
            selectSoupElement('price', soup, SoupSearchObj('span', {"aria-label": "Price"}, True, 0),
                              ['text']))

        apInfo.ccy = selectSoupElement('ccy', soup,
                                             SoupSearchObj('span', {"aria-label": "Currency"}, True, 0),
                                             ['text'])

        apInfo.refNo = selectSoupElement('refNo', soup,
