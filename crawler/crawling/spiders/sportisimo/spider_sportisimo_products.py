...
class SportisimoProductsSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        urls = map(lambda x: x.a["href"], soup.find_all("div", class_="product-box__name"))
        return self.generateSpiders(response, NextSpidersInfo("spider_sportisimo_product_details", list(urls), config.flowTimeouts.get(self.name)))
