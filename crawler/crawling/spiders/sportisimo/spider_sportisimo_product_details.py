...
class SportisimoProductDetailsSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        shoeInfo.productCode = selectSoupElement('productCode', soup, SoupSearchObj('span', {"id": 'product_code_p%s_spec' % id}), ['text'])
        shoeInfo.url = response.request.url
        shoeInfo.supplierCode = selectSoupElement('supplierCode', soup, SoupSearchObj('span', {"id": 'product_match_code_p%s' % id}), ['text'])
        shoeInfo.shoeName = selectSoupElement('shoeName', soup,
                                                  SoupSearchObj('td', {"class": 'p_title'}),
                                                  ['h1', 'span', 'text'])
        shoeInfo.price = getPreciseNumber(selectSoupElement('price', soup,
                                                  SoupSearchObj('p', {"class": 'price'}),
                                                  ['text']))
