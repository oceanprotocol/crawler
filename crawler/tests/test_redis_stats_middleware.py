...
class TestRedisStatsMiddleware(TestCase):
    @mock.patch("crawling.redis_stats_middleware" ".RedisStatsMiddleware.setup")
    def setUp(self, s):
    ...
    def test_load_stats_codes(self):
        ...
        self.assertEqual(
            list(self.rsm.stats_dict[spider_name]["status_codes"].keys()), []
        )
        ...
        self.assertEqual(
            list(self.rsm.stats_dict[spider_name]["status_codes"][200].keys()),
            ["lifetime"],
        )
        self.assertEqual(
            list(self.rsm.stats_dict[spider_name]["status_codes"][403].keys()),
            ["lifetime"],
        )
        ...
        self.assertEqual(
            sorted(
                [
                    str(x)
                    for x in self.rsm.stats_dict[spider_name]["status_codes"][
                        200
                    ].keys()
                ]
            ),
    ...
    def test_rsm_input(self):
        ...
        self.rsm.settings["STATS_TIMES"] = ["lifetime"]
        ...
        self.rsm.stats_dict = {
            "link": {
                "status_codes": {
                    200: {"lifetime": fake_stats},
                    400: {"lifetime": fake_stats},
                    404: {"lifetime": fake_stats},
                }
            },
            "wandering": {
                "status_codes": {
