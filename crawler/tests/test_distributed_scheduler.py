...
class ThrottleMixin(object):
    @mock.patch(
        "crawling.distributed_scheduler.DistributedScheduler" ".update_ipaddress"
    )
    @mock.patch(
        "crawling.distributed_scheduler.DistributedScheduler" ".setup_zookeeper"
    )
    def setUp(self, u, z):
...
class TestDistributedSchedulerEnqueueRequest(ThrottleMixin, TestCase):
    ...
    def test_enqueue_request(self, t):
        ...
        self.scheduler.global_page_per_domain_filter.request_page_limit_reached = (
            MagicMock(return_value=False)
        )
        self.scheduler.domain_max_page_filter.request_page_limit_reached = MagicMock(
            return_value=False
        )
...
class TestDistributedSchedulerFindItem(ThrottleMixin, TestCase):
    def test_find_item(self):
        ...
        self.scheduler.queue_keys = ["link:ex.com:queue"]
        self.scheduler.queue_dict = {"link:ex.com:queue": [MagicMock(), 0]}
        self.scheduler.queue_dict["link:ex.com:queue"][0].pop = MagicMock(
            return_value="item"
        )
...
class TestDistributedSchedulerNextRequest(ThrottleMixin, TestCase):
    ...
    def test_next_request(self, t):
        ...
        self.req.meta[
            "cookie"
        ] = "authenticated=true;privacy=10"  # add cookie to req since we are not testing this
        self.scheduler.find_item = MagicMock(return_value=feed)
        out = self.scheduler.next_request()
        self.assertEqual(out.url, "http://ex.com")
        for key in out.meta:
...
class TestDistributedSchedulerLoadDomainConfig(ThrottleMixin, TestCase):
    def test_load_domain_config(self):
        good_yaml_dict = {
            "blacklist": ["blah.com"],
            "domains": {
                "ex1.com": {"window": 60, "hits": 10, "scale": 1},
                "ex2.com": {"window": 60, "hits": 10},
            },
        }
        bad_yaml_dict1 = {"blarg": ["stuff"]}
        bad_yaml_dict2 = {
            "domains": {"stuff.com": {"window": 60}, "stuff2.com": {"hits": 10}}
        }
        result_yaml = {}
...
class TestDistributedSchedulerUpdateDomainQueues(ThrottleMixin, TestCase):
    def test_update_domain_queues(self):
        ...
        self.scheduler.domain_config = {
            "ex1.com": {"window": 50, "hits": 10, "scale": 1}
        }
        ...
        self.scheduler.domain_config = {
            "ex2.com": {"window": 50, "hits": 10, "scale": 0.5}
        }
        q = RedisThrottledQueue(MagicMock(), MagicMock(), 100, 100)
...
class TestDistributedSchedulerCreateQueues(ThrottleMixin, TestCase):
    def test_create_queues(self):
        ...
        expected = ["ex1:throttle_window", "ex2:throttle_window", "ex3:throttle_window"]
        for key in self.scheduler.queue_dict:
            self.assertTrue(self.scheduler.queue_dict[key][0].window_key in expected)

        # test type
        self.scheduler.add_type = True
        self.scheduler.queue_dict = {}
        expected = [
            "link:ex1:throttle_window",
...
class TestDistributedSchedulerParseCookie(ThrottleMixin, TestCase):
    def test_parse_cookie(self):
        cookie = (
            "blah=stuff; expires=Thu, 18 May 2017 12:42:29 GMT; path"
            "=/; Domain=.domain.com"
        )
        result = {
            "blah": "stuff",
