...
class ItemMixin(object):
    ...
    def _get_internationalized_utf8_item(self):
        ...
        item["response_headers"] = {}
        item["request_headers"] = {}
        item["body"] = "This is a test - Αυτό είναι ένα τεστ - 这是一个测试 - これはテストです"
        item["links"] = []
        item["encoding"] = "utf-8"
...
class TestKafkaPipeline(TestCase, ItemMixin):
    ...
    def test_process_item(self, e):
        ...
        self.pipe.process_item(item, spider)
        self.pipe.producer.send.assert_called_with("prefix.crawled_app", expected)
        self.pipe.producer.send.reset_mock()
        ...
        self.pipe.producer.send.assert_called_with("prefix.crawled_firehose", expected)
        ...
        self.assertEquals(decoded_string, "αυτό είναι ένα τεστ\n")
