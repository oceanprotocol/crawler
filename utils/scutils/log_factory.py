...
class LogObject(object):
    ...
    def __init__(
        ...
        if stdout:
            ...
            file_handler = ConcurrentRotatingFileHandler(
                dir + "/" + file, maxBytes=bytes, backupCount=backups
            )
            ...
            self._check_log_level(level)
            self.debug("Logging to file: {file}".format(file=dir + "/" + file))
    ...
    def _check_log_level(self, level):
        ...
        if level not in list(self.level_dict.keys()):
            ...
            self.logger.warn(
                "Unknown log level '{lev}', defaulting to DEBUG".format(lev=level)
            )
