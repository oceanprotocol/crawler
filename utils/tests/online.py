...
from scutils.stats_collector import (
    ThreadedCounter,
    TimeWindow,
    RollingTimeWindow,
    Counter,
...
class TestStatsThreaded(RedisMixin, TestCase):
    ...
    def test_purge_old(self):
        ...
        tc.purge_old()
        self.assertEqual(
            ["default_counter:2015-10"], self.redis_conn.keys(tc.get_key() + ":*")
        )
...
class TestStatsTimeWindow(RedisMixin, TestCase, CleanMixin):
    def test_window(self):
        counter = TimeWindow(key="test_key", cycle_time=0.1, window=1)
    ...
    def test_roll_window(self):
        counter = TimeWindow(key="test_key", cycle_time=0.01, window=1, roll=True)
        counter.setup(self.redis_conn)
        counter.increment()
        counter.increment()
        counter.increment()
...
class TestStatsRollingTimeWindow(RedisMixin, TestCase, CleanMixin):
    def test_rolling_window(self):
        ...
        counter = RollingTimeWindow(key="test_key", cycle_time=0.01, window=5)
        counter.setup(self.redis_conn)
        counter.increment()
        counter.increment()
        time.sleep(3)
...
class TestStatsCounter(RedisMixin, TestCase, CleanMixin):
    ...
    def test_roll_generic_counter(self):
        ...
        counter = Counter(key="test_key", window=2, cycle_time=0.1, roll=True)
        counter.setup(redis_conn=self.redis_conn)
        i = 0
        while i < 100:
            counter.increment()
...
class TestStatsUniqueCounter(RedisMixin, TestCase, CleanMixin):
    ...
    def test_roll_uniques(self):
        # rough sleep to get us back on track
        time.sleep(2 - (int(time.time()) % 2))
        counter = UniqueCounter(key="test_key", window=2, cycle_time=0.1, roll=True)
        counter.setup(redis_conn=self.redis_conn)
        i = 0
        while i < 100:
...
class TestStatsBitMapCounter(RedisMixin, TestCase, CleanMixin):
    ...
    def test_roll_bitmap_counter(self):
        # rough sleep to get us back on track
        time.sleep(2 - (int(time.time()) % 2))
        counter = BitMapCounter(key="test_key", window=2, cycle_time=0.1, roll=True)
        counter.setup(redis_conn=self.redis_conn)
...
class TestZookeeperWatcher(TestCase):
    ...
    def setUp(self):
        ...
        self.zoo_client.set(self.pointer_path, self.pointer_data.encode("utf-8"))
    ...
    def test_get_file_contents(self):
        pointer_zoo_watcher = ZookeeperWatcher(
            hosts=self.hosts,
        ...
        self.assertEqual(pointer_zoo_watcher.get_file_contents(), self.file_data)
        self.assertEqual(pointer_zoo_watcher.get_file_contents(True), self.pointer_data)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Online deployment Test" " Script for Utils"
    )
    parser.add_argument(
        "-r",
        "--redis-host",
    ...
    parser.add_argument(
        "-p", "--redis-port", action="store", default="6379", help="The Redis port"
    )
    parser.add_argument(
        "-P",
        "--redis-password",
    ...
    suite.addTest(TestZookeeperWatcher("test_get_file_contents", args["zoo_keeper"]))
