...
class KafkaMonitor(object):
    ...
    def _setup_stats(self):
        ...
        try:
            ...
            self.logger.warn(
                "Failed to connect to Redis in StatsCollector"
    ...
    def _setup_stats_total(self, redis_conn):
        ...
        for item in self.settings["STATS_TIMES"]:
            try:
                ...
                self.logger.info("Unable to find Stats Time '{s}'".format(s=item))
            ...
            for message in self.consumer:
                ...
                try:
                    ...
                    if not found_plugin:
                        ...
                        self.logger.warn(
                            "Did not find schema to validate " "request", extra=extras
                        )
                    ...
                    self.logger.info("Unparseable JSON Received", extra=extras)
    ...
    def _increment_plugin_stat(self, name, item):
        ...
        if "plugins" in self.stats_dict:
            self.logger.info("Incremented plugin '{p}' plugin stats".format(p=name))
    ...
    def feed(self, json_item):
        ...
        def _feed(json_item):
            ...
            if not self.logger.json:
                ...
                self.logger.info(
                    "Feeding JSON into {0}\n".format(topic), extra={"value": json_item}
                )
    ...
    def _create_producer(self):
        ...
        try:
            brokers = self.settings["KAFKA_HOSTS"]
            self.logger.info(
                "Creating new kafka producer using brokers: " + str(brokers)
            )
            ...
            self.logger.info(
                "Couldn't initialize kafka producer.", {"ex": traceback.format_exc()}
            )
            raise
...
def main():
    ...
    run_parser = subparsers.add_parser(
        "run", help="Run the Kafka Monitor", parents=[base_parser]
    )
