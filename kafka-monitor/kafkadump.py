...
def main():
    ...
    list_parser = subparsers.add_parser(
        "list", help="List all Kafka topics", parents=[base_parser]
    )

    # dump command
    dump_parser = subparsers.add_parser(
        "dump", help="Dump a Kafka topic", parents=[base_parser]
    )
    ...
    dump_parser.add_argument(
        ...
        const=True,
        default=False,
        help="Do not include the raw html 'body' key in" " the json dump of the topic",
    )
    dump_parser.add_argument(
        "-jb",
        "--just-body",
        ...
        const=True,
        default=False,
        help="Just print the raw html 'body' key in" " the json dump of the topic",
    )
    dump_parser.add_argument(
        "-p",
        "--pretty",
    ...
    if args["command"] == "list":
                ...
                for message in consumer:
                    ...
                    try:
                        ...
                        if args["decode_base64"] and "body" in item:
                            item["body"] = base64.b64decode(item["body"]).decode(
                                encoding
                            )
        ...
        if num_records > 0:
            logger.info(
                "Num Records: {n}, Total MBs: {m}, kb per message: {kb}".format(
                    n=num_records,
                    m=total_mbs,
