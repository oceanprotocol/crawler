...
class StatsMonitor(KafkaBaseMonitor):
    ...
    def handle(self, key, value):
        ...
        if stats == "all":
            ...
            self.logger.warn(
                "Received invalid stats request: {s}".format(s=stats), extra=extras
            )
            return
    ...
    def _get_plugin_stats(self, name):
        ...
        for key in keys:
            ...
            if main == "total" or main == "fail":
                ...
                the_dict["plugins"][main][end] = self._get_key_value(
                    key, end == "lifetime"
                )
    ...
    def get_spider_stats(self):
        ...
        for key in keys:
            ...
            if len(elements) == 6:
                ...
                the_dict[spider][response][end] = self._get_key_value(
                    key, end == "lifetime"
                )
    ...
    def get_machine_stats(self):
        ...
        for key in keys:
            ...
            if end in the_dict[machine][response]:
                the_dict[machine][response][end] = the_dict[machine][response][
                    end
                ] + self._get_key_value(key, end == "lifetime")
            else:
                the_dict[machine][response][end] = self._get_key_value(
                    key, end == "lifetime"
                )
    ...
    def get_queue_stats(self):
        ...
        for key in keys:
            ...
            the_dict[spider]["domains"].append({"domain": domain, "backlog": count})
