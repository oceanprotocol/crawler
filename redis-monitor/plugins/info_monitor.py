...
class InfoMonitor(KafkaBaseMonitor):
    ...
    def handle(self, key, value):
        ...
        extras = self.get_log_dict(
            "info", dict["appid"], dict["spiderid"], master["uuid"]
        )

        if len(elements) == 4:
            dict["crawlid"] = elements[3]
            extras = self.get_log_dict(
                "info", dict["appid"], dict["spiderid"], master["uuid"], elements[3]
            )
        ...
        if self._send_to_kafka(master):
            ...
            self.logger.error("Failed to send info to kafka", extra=extras)
            ...
            for score in sortedDict:
                for item in sortedDict[score]:
                    ...
                    if item["appid"] == dict["appid"]:
                        ...
                        master["crawlids"][crawlid]["total"] = (
                            master["crawlids"][crawlid]["total"] + 1
                        )
                        ...
                        master["crawlids"][crawlid]["domains"][domain]["total"] = (
                            master["crawlids"][crawlid]["domains"][domain]["total"] + 1
                        )
    ...
    def _build_crawlid_info(self, master, dict):
        ...
        timeout_key = "timeout:{sid}:{aid}:{cid}".format(
            sid=dict["spiderid"], aid=dict["appid"], cid=dict["crawlid"]
        )
            ...
            for score in sortedDict:
                for item in sortedDict[score]:
                    if "meta" in item:
                        item = item["meta"]
                    if (
                        item["appid"] == dict["appid"]
                        and item["crawlid"] == dict["crawlid"]
                    ):
                        ...
                        master["domains"][domain]["total"] = (
                            master["domains"][domain]["total"] + 1
                        )

                        if (
                            item["priority"]
                            > master["domains"][domain]["high_priority"]
                        ):
                            master["domains"][domain]["high_priority"] = item[
                                "priority"
                            ]
