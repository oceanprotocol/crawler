...
class TestInfoPlugin(TestCase, RegexFixer):
    ...
    def test_info_regex(self):
        ...
        self.assertEqual(
            re.findall(regex, "info:stuff:stuff:stuff"), ["info:stuff:stuff:stuff"]
        )
    ...
    def test_info_get_crawlid(self):
        ...
        self.plugin._get_bin = MagicMock(
            return_value={
                -200: [{"appid": "testapp", "priority": 10, "crawlid": "crawlIDHERE"}]
            }
        )
        ...
        success = {
            ...
            "appid": "testapp",
            "domains": {
                "bingo.com": {"low_priority": 10, "high_priority": 10, "total": 1}
            },
            "uuid": "ABC123",
        }
...
class TestStopPlugin(TestCase, RegexFixer):
    ...
    def test_stop_regex(self):
        regex = self.fix_re(self.plugin.regex)
        self.assertEqual(
            re.findall(regex, "stop:spider:app:crawl"), ["stop:spider:app:crawl"]
        )
        self.assertEqual(re.findall(regex, "stop:spider:app"), ["stop:spider:app"])
        self.assertEqual(re.findall(regex, "stop:stuff"), [])

    def test_stop_monitor_mini_purge(self):
        self.plugin.redis_conn.scan_iter = MagicMock(
            return_value=["link:istresearch.com:queue"]
        )
...
class TestExpirePlugin(TestCase, RegexFixer):
    ...
    def test_stop_regex(self):
        regex = self.fix_re(self.plugin.regex)
        self.assertEqual(
            re.findall(regex, "timeout:blah1:blah2:bla3"), ["timeout:blah1:blah2:bla3"]
        )
...
class TestStatsPlugin(TestCase, RegexFixer):
    ...
    def test_stats_regex(self):
        regex = self.fix_re(self.plugin.regex)
        self.assertEqual(
            re.findall(regex, "statsrequest:crawler:testApp"),
    ...
    def test_stats_get_machine(self):
        ...
        self.plugin.redis_conn.keys = MagicMock(
            return_value=[
                "stats:crawler:host1:link:200:3600",
                "stats:crawler:host2:link:200:lifetime",
                "stats:crawler:host1:link:504:86400",
    ...
    def test_stats_get_plugin(self):
        ...
        good = {
            "plugins": {"pluginX": {"lifetime": 5, "86400": 5}},
...
class TestZookeeperPlugin(TestCase, RegexFixer):
    ...
    def test_zk_regex(self):
        regex = self.fix_re(self.plugin.regex)
        self.assertEqual(
            re.findall(regex, "zk:blah1:blah2:bla3"), ["zk:blah1:blah2:bla3"]
        )
