...
class TestRedisMonitor(TestCase):
    ...
    def test_load_plugins(self):
        ...
        self.redis_monitor.settings["PLUGINS"][
            "plugins.stop_monitor.StopMonitor"
        ] = None
        self.redis_monitor._load_plugins()
        self.assertEqual(list(self.redis_monitor.plugins_dict.keys()), assert_keys)
        ...
        self.redis_monitor.settings["PLUGINS"]["plugins.crazy_class.CrazyHandler"] = (
            400,
        )
    ...
    def test_active_plugins(self):
        ...
        list(self.redis_monitor.plugins_dict.items())[0][1][
            "instance"
        ].handle = MagicMock(side_effect=BaseException("info"))
        list(self.redis_monitor.plugins_dict.items())[1][1][
            "instance"
        ].handle = MagicMock(side_effect=BaseException("stop"))
        list(self.redis_monitor.plugins_dict.items())[2][1][
            "instance"
        ].handle = MagicMock(side_effect=BaseException("expire"))
        ...
        try:
            list(self.redis_monitor.plugins_dict.items())[0][1][
                "instance"
            ].handle = MagicMock(side_effect=Exception("normal"))
            plugin = list(self.redis_monitor.plugins_dict.items())[0][1]
            self.redis_monitor._process_plugin(plugin)
    ...
    def test_load_stats_plugins(self):
        ...
        self.assertEqual(
            sorted(self.redis_monitor.stats_dict["plugins"].keys()), sorted(defaults)
        )
        ...
        for key in self.redis_monitor.plugins_dict:
            ...
            self.assertEqual(
                sorted(
                    [
                        str(x)
                        for x in self.redis_monitor.stats_dict["plugins"][
                            plugin_name
                        ].keys()
                    ]
                ),
    ...
    def test_process_failures(self):
        self.redis_monitor.settings = {"RETRY_FAILURES": True, "RETRY_FAILURES_MAX": 3}
